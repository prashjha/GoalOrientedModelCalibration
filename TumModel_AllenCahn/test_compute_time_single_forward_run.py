...
run_dir = 'test_compute_time_single_forward_run/'
theta0 = [0.2, 0.1, 0.05]
dt = 0.005
use_approx = True
which_est = 0
...
if __name__ == "__main__":
    ...
    t1 = time.time()
    pde.solveFwdBck(x, True, False, True)
    t2 = time.time()
    print("\nNonlinear solve in %s seconds ---" % (t2 - t1))

    ## solve approximate error to compute QoI
    print("\n\nSolve for approximate errors\n")
    t1 = time.time()
    pde.solveFwdBckErr(x, True, False)
    t2 = time.time()
    print("\nLinear error solve in %s seconds ---" % (t2 - t1))
